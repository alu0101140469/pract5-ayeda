...
// Función auxiliar que imprime los elementos de la secuencia (usada en la traza de los algoritmos)
...
void shakeSort(Sequence<Key>& seq, bool trace) {
    bool swapped = true;         // Bandera para ver si hubo intercambios.
    unsigned start = 0;          // Índice inicial del recorrido.
    unsigned end = seq.getSize() - 1;  // Índice final del recorrido.
    unsigned iteration = 0;      // Contador de iteraciones (para traza).
    while (swapped) {
        swapped = false;
...
void shellSort(Sequence<Key>& seq, double alfa, bool trace) {
    ...
    while ((gap = int(gap * alfa)) > 0) {
        ...
        for (int i = gap; i < n; i++) {
            Key temp = seq[i];
            int j = i;
            // Desplaza elementos que están 'gap' posiciones detrás si son mayores que temp.
